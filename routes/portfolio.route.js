...
router.post("/buy-player", authMiddleware, async (req, res) => {
    try {
        ...
        company.profitFromPlatformFees += onePercent
...
router.post("/sell-player", authMiddleware, async (req, res) => {
    try {
        ...
        if (companyShare > 0) {
            ...
            company.profitFromProfitableCuts += companyShare;
            await company.save();
        }
...
router.get("/all", authMiddleware, async (req, res) => {
    try {
        ...
        // Only fetch last 10 portfolios (most recent)
        // Sort portfolios by timestamp descending and take the latest 10
        ...
        // Calculate total deposited amount (sum of all completed deposits)
        ...
        // The current value is the user's current amount
        const value = user.amount;

        // Profit is the difference between current amount and total deposited
        const profit = value - depositedAmount;
